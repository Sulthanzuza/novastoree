...
const loadHome = async (req, res) => {
  try {
    ...
    res.status(200).render('user/home', {
      ...
      wishlistProductIds,
      isLoggedIn, 
      user: req.session.user || req.session.tempUser, 
      sessionType: isLoggedIn ? 'permanent' : 'temporary' 
    });
...
const showProductDetails = async (req, res) => {
  ...
  // Use your existing guest session technique
  const userId = req.session.user ? req.session.user.id : null;
  const isLoggedIn = !!userId;
  ...
  try {
      ...
      // Set selected variant for similar products
      ...
      // Handle wishlist only for logged-in users
      let userWishlist = [];
      let wishlistProductIds = {};

      if (isLoggedIn) {
          userWishlist = await Wishlist.find({ userId: userId });
      ...
      // Apply discounts to main product and similar products
      await applyDiscountsToProducts(products);
      await Promise.all(similarProducts.map(applyDiscountsToProducts));

      // Handle order history and purchase status only for logged-in users
      let userOrders = [];
      ...
      if (isLoggedIn) {
          ...
          userHasOrdered = userOrders.some(order => 
      ...
      // Get reviews (available for both guests and logged-in users)
      ...
      return res.status(200).render('user/products-details', {
        ...
        isLoggedIn: isLoggedIn, // Boolean flag for authentication status
        tempUser: req.session.tempUser || null, // Pass temp user data
        products,
        similarProducts,
        wishlistProductIds,
        wishListIds,
...
const showListProducts = async (req, res) => {
  try {
    ...
    const actualUserId = req.session.user ? req.session.user.id : null;
         
    let wishlistProductIds = {};
    
    if (actualUserId) {
      const wishlistItems = await Wishlist.find({ userId: actualUserId }).lean();
      wishlistItems.forEach(item => {
          wishlistProductIds[item.variantId.toString()] = true;
      });
    }
